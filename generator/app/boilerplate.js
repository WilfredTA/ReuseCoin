...
const udtDefPath = "./ckb-scripts/build/udt_def"
const udtInfoTypePath = "./ckb-scripts/build/udt_info_type"
const udtInfoLockPath = "./ckb-scripts/build/udt_info_lock"
let specialCells = {}
...
const init = async () => {
    ...
    const generateTransaction = async (outputs, additional = {deps: [], inputs: []}) => {
      ...
      console.log(rawTx.outputs[rawTx.outputs.length - 1].capacity, "<< CHANGE CAPACITY")

      let oldCapacity = rawTx.outputs[rawTx.outputs.length - 1].capacity
      rawTx.outputs[rawTx.outputs.length - 1].capacity = BigInt(Math.floor(Number(oldCapacity) / 4))
