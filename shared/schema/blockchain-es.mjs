...
export function SerializetypeId(value) {
  ...
  buffers.push(SerializeUint32(value.idx));
  return serializeTable(buffers);
}

export class UDTData {
  constructor(reader, { validate = true } = {}) {
    this.view = new DataView(assertArrayBuffer(reader));
    if (validate) {
      this.validate();
...
export function SerializeUDTData(value) {
  const buffers = [];
  buffers.push(SerializeUint128(value.amount));
  return serializeTable(buffers);
}

export class ReuseCoinArgs {
  constructor(reader, { validate = true } = {}) {
    this.view = new DataView(assertArrayBuffer(reader));
    if (validate) {
      this.validate();
...
export function SerializeReuseCoinArgs(value) {
  const buffers = [];
  buffers.push(SerializeByte32(value.wallet_hash));
  return serializeTable(buffers);
}
...
export function SerializeReuseCoinWalletArgs(value) {
  const buffers = [];
  buffers.push(SerializeByte20(value.pubkey_hash));
